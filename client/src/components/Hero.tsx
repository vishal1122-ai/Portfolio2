...
export default function Hero() {
  const [currentAnimation, setCurrentAnimation] = useState<AnimationType>('neural');
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (typeof window !== 'undefined' && window.gsap) {
        ...
        gsap.utils.toArray('.text-reveal-inner').forEach((element: any, index: number) => {
          gsap.fromTo(element,
            { opacity: 0 },
            { 
              opacity: 1,
    ...
    <section id="home" className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-4xl mx-auto px-6">
        <div className="text-reveal mb-6">
          <h1 className="text-reveal-inner text-6xl md:text-8xl font-bold mb-4">
        ...
        <div className="text-reveal">
          <button 
            ...
            className="text-reveal-inner bg-gradient-to-r from-electric to-neon px-8 py-4 rounded-full text-white font-semibold hover:scale-105 transform transition-all duration-300 shadow-lg hover:shadow-xl"
          >
            Let's Connect
          </button>
        </div>
